...
function Selector({ values = testValues, title, isRadio, inQuickFilter, name }: FilterSelectProps) {
  ...
  function showOptions() {
    return values?.map((value, index) =>
      isRadio ? (
        ...
        <Checkbox key={index} text={value} isInSelect name={name} />
      ),
    )
  }
  ...
  return (
    <div
      className={`group relative select-none border-b border-b-base-600/10 bg-base-100 pb-[18px] first:border-t first:border-t-base-600/10 first:pt-[18px] md:w-[260px] md:rounded-[16px] md:border-b-0 md:px-[16px] md:py-[12px] md:first:border-t-0 ${openedClasses}`}
