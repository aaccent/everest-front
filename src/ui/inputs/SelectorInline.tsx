...
function SelectorInline({ name, list, initValue, className }: Props) {
  ...
  function showItems(list: Props['list']) {
    return list.map((value, i) => {
      ...
      return (
        <button
          className={`text-base-400-lg-100 peer relative rounded-[14px] px-[16.5px] py-[6px] transition-colors before:absolute before:left-0 before:top-1/2 before:h-[12px] before:w-[1px] before:-translate-y-1/2 before:bg-base-400 before:transition-colors first-of-type:before:hidden hover:bg-base-300 hover:before:bg-transparent previous-has-[.active]:before:bg-transparent previous-has-[:hover]:before:bg-transparent ${isActive ? 'active bg-base-300 before:bg-transparent' : ''}`}
  ...
  return (
    <div
      className={`flex items-center gap-[2px] rounded-[20px] bg-base-100 px-[8px] py-[7px] md:rounded-[16px] ${className}`}
    >
