...
function Checkbox({ text, isInSelect, name, onClick }: CheckboxProps) {
  const clickHandle = () => {
    if (!onClick || !text) return
    onClick(text)
  }
  return isInSelect ? (
    <label className='text-base-500-reg-100-upper flex cursor-pointer items-center gap-[10px]' onClick={clickHandle}>
    ...
    <label
      className='text-base-100-reg-100 md:text-base-500-reg-100-upper flex cursor-pointer items-center justify-between border-b border-t border-b-base-600/10 border-t-base-600/10 py-[16px] md:gap-[10px] md:border-none md:py-0'
      onClick={clickHandle}
    >
