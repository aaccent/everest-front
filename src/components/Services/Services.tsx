  ...
  return (
    <>
      {services.map((service) => (
        ...
        >
          <div
            className={`mr-[14px] flex size-[52px] items-center justify-center rounded-full bg-base-115 md:mr-[20px] md:size-[80px]`}
          >
            <Image src={service.icon} alt={''} width={34} height={34} className={`size-[22px] md:size-[34px]`} />
          </div>
          <div>{service.title}</div>
          <div
            className={`absolute right-[18px] flex size-[52px] items-center justify-center rounded-full border border-base-115 after:block after:size-[20px] after:rotate-45 after:bg-icon-arrow-up after:filter-base-100 after:bg-default group-hover:border-base-100 group-hover:bg-base-100 group-hover:after:filter-primary md:right-[24px] md:size-[80px]`}
          ></div>
        </Link>
      ))}
    </>
