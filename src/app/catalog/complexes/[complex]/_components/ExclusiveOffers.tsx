...
import { getExclusiveOffers } from '@/globals/api'
...
async function ExclusiveOffers() {
  ...
  function showOffers() {
    return offersBlock.items.map((offer) => {
      return (
        <li key={offer.id} className='relative w-full rounded-[24px] bg-base-100/10 p-[20px] md:p-[24px]'>
          <div className='mb-[12px] flex w-full justify-between md:mb-[33px]'>
            <div className='text-base-100-reg-100 max-w-[200px] text-base-100'>{offer.title}</div>
            <Img src={logo} className='w-full max-w-[32px] opacity-20 md:max-w-[49px]' />
          </div>
          <div className='text-base-100/50'>{offer.text}</div>
